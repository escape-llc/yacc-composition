...
namespace eScapeLLC.UWP.Charts.Composition {
	...
	#endregion
	#region CategoryValueSeries<S>
	/// <summary>
	/// Base class for <see cref="DataSourceSeries"/> based on 2 components (category, value).  Category is an Index component.
	/// </summary>
	...
	public abstract class CategoryValue_VisualPerItem<S> : CategoryValueSeries<S>, IOperationController<S> where S: ItemState_CategoryValue<Visual> {
		...
		protected virtual void Entering(S item, ItemTransition it) {
			...
			if (Animate != null) {
				...
				Animate.Enter(ctx, item.Element, Container.Children);
			}
			else {
		...
		protected virtual void Exiting(S item, ItemTransition it) {
			...
			if (Animate != null) {
				var ctx = CreateAnimateContext(item, it);
				Animate.Exit(ctx, item.Element, Container.Children, co => {
					item.ResetElement();
				});
			}
			else {
		...
		protected virtual void UpdateOffset(S item) {
			...
			if (Animate != null) {
				...
				item.Element.Offset = new Vector3(offset, 0);
			}
		}
		#endregion

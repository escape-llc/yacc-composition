...
namespace eScapeLLC.UWP.Charts.Composition {
	...
	public static class AnimationProperties {
		...
		public readonly static string POSITION_X = $"({PARM_local}.Position.X * {PARM_global}.ModelX.X + {PARM_global}.ModelX.Z) * {PARM_global}.ProjX.X + {PARM_global}.ProjX.Z";
		public readonly static string POSITION_Y = $"({PARM_local}.Position.Y * {PARM_global}.ModelY.Y + {PARM_global}.ModelY.Z) * {PARM_global}.ProjY.Y + {PARM_global}.ProjY.Z";
	...
	/// </summary>
	public class Animation_MarkerBrush : IDisposable {
		/// <summary>
		...
		/// <param name="cx">Use to obtain objects.</param>
		/// <param name="name">Use for names.</param>
		/// <param name="props">Use for GLOBAL property set.</param>
		public Animation_MarkerBrush(Compositor cx, string name, CompositionPropertySet props) {
			CreateAnimations(cx, name, props);
		}
		#endregion
		#region extensions
		protected virtual void CreateAnimations(Compositor cx, string name, CompositionPropertySet global) {
			Position = cx.CreateVector2KeyFrameAnimation();
			Position.Comment = $"Marker[{GetHashCode()}]_Position";
			...
			PropertySet = cx.CreatePropertySet();
			PropertySet.Comment = $"Marker[{GetHashCode()}]_PropertySet";
